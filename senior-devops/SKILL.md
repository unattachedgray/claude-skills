...
For complex multi-stage deployments and CI/CD pipelines that need crash recovery, consider [Temporal](https://github.com/temporalio/temporal):

- **When to use:** Multi-step deployment pipelines (build → test → stage → canary → promote), rollback orchestration across multiple services, infrastructure provisioning with approval gates, long-running migration workflows
- **When to skip:** Simple linear CI/CD (GitHub Actions/Jenkins suffice), single-service deploys, cron jobs
- **Key benefit:** Workflows resume from the exact failed step after crashes — no re-running completed stages. Built-in retries, timeouts, and visibility into workflow state.
- **Alternative:** For simpler needs, a job queue (BullMQ, Celery) or step-function service (AWS Step Functions) may suffice with less infrastructure overhead.
