...
## Layout Verification Evals (frontend skills)

For skills that generate web UI, add layout verification checks:

```markdown
## Expected
- [ ] Output contains no `text_multiline_overflow` issues
- [ ] Output contains no `label_overflow` issues
- [ ] All buttons pass `verify_layout` with maxLines:1
```

Use `read_page` with `layoutCheck:true` as the verification step. See `/pretext-layout` for full patterns.
